...
{
	//gives you the beginning address of the block number you specity
...
{
	uint32_t i;
	ospfs_inode_t *inode_ptr;
	for (i =2; i< ospfs_super->os_ninodes; i++)
	{
		inode_ptr = ospfs_inode(i);
		if(inode_ptr->oi_nlink == 0)
			return i;
	}
	return 0;
...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	while (r == 0 && ok_so_far >= 0 && f_pos >= 2) {
		...
		if(od->od_ino){
			//given an inode number we return the pointer to the inode
...
add_block(ospfs_inode_t *oi)
{
	...
	//eprintk("function add_block is used\n");
	...
	//eprintk("Newly allocated block is %d\n", new_block);
	//eprintk("Need %d blocks\n", n);

	if(new_block > 0){
		//zero out all the blocks 
		///eprintk("Zero out blocks\n");
		memset(ospfs_block(new_block), 0, OSPFS_BLKSIZE);
	}
	else {
		//eprintk("Allocate_block() failed\n");
		return -ENOSPC;
	}


	if(indir2_index(n) == 0){
		//eprintk("Need indirect2 blocks\n");
		if(n == OSPFS_NDIRECT + OSPFS_NINDIRECT){
                	new_allocated_indirect2 = allocate_block();
                	if(new_allocated_indirect2 == 0){
                        	free_block(new_block);
                        	return -ENOSPC;
                	}
               		 //zero out the newly allocated block
                	memset(ospfs_block(new_allocated_indirect2),0, OSPFS_BLKSIZE);
                	oi->oi_indirect2 = new_allocated_indirect2;
       		}
		uint32_t *indirect2_ptr = ospfs_block(oi->oi_indirect2);;

		if((n-OSPFS_NDIRECT-OSPFS_NINDIRECT) % OSPFS_NINDIRECT == 0)
		{
			new_allocated_indirect = allocate_block();
			if(new_allocated_indirect == 0)
			{
				free_block(new_block);
				return -ENOSPC;
			}
			memset(ospfs_block(new_allocated_indirect),0,OSPFS_BLKSIZE);
			*(indirect2_ptr+indir_index(n)) = new_allocated_indirect;
		}	
		else
		{
			new_allocated_indirect = indirect2_ptr[indir_index(n)];
		}
		uint32_t *indirect_ptr = ospfs_block(new_allocated_indirect);
		indirect_ptr[(n-OSPFS_NDIRECT-OSPFS_NINDIRECT)%OSPFS_NINDIRECT]=new_block;
	}
	else if(n >= OSPFS_NDIRECT){
		//we are adding one block each time so once we need to add 
		//indiret block it allocates
		//eprintk("Need indirect block\n");
		if(n == OSPFS_NDIRECT){
			new_allocated_indirect = allocate_block();
			if(new_allocated_indirect == 0){
				//some problem happens. Free the already allocated block
				free_block(new_block);
				return -ENOSPC;
			}
			memset(ospfs_block(new_block), 0, OSPFS_BLKSIZE);
		...
		//get the address of indirect block
		uint32_t *indirect_ptr = ospfs_block(oi->oi_indirect);
		indirect_ptr[direct_index(n)] = new_block;
	}
	else { //n < OSPFS_NDIRECT
		//eprintk("Only need direct blocks\n");
		oi->oi_direct[n] = new_block;
	}
	oi->oi_size += OSPFS_BLKSIZE;
	//eprintk("Just update the file size to %d, finish add_block\n", oi->oi_size);
	return 0;
}
...
{
	// current number of blocks in file
	//eprintk("Function remove_block is used\n");
	uint32_t n = ospfs_size2nblocks(oi->oi_size);
	//eprintk("We gonna remove %d number of blocks\n", n);
	...
	if(n >= OSPFS_NDIRECT + OSPFS_NINDIRECT){
		...
		uint32_t *indirect_ptr = ospfs_block(indirect2_ptr[indir_index(n)]);

		free_block(indirect_ptr[(n-OSPFS_NDIRECT-OSPFS_NINDIRECT)%OSPFS_NINDIRECT]);
		indirect_ptr[(n-OSPFS_NDIRECT-OSPFS_NINDIRECT)%OSPFS_NINDIRECT]=0;
		if((n-OSPFS_NDIRECT-OSPFS_NINDIRECT)%OSPFS_NINDIRECT == 1){
			free_block(indirect2_ptr[indir_index(n)]);
			indirect2_ptr[indir_index(n)]=0;
	...
	//	eprintk("Need to remove a block from direct link list\n");
...
{
	//eprintk("Function change_size is used\n");
	uint32_t old_size = oi->oi_size;
	int r = 0;
	//eprintk("Initial size is %d\n", old_size);
	//eprintk("We want to change the size to %d\n", new_size);
	...
	while (ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size)) {
	        /* EXERCISE: Your code here */
		r = add_block(oi);	
		//eprintk("Update the size to %d\n",oi->oi_size);
		if(r == -ENOSPC){	
			
			while(ospfs_size2nblocks(oi->oi_size) > ospfs_size2nblocks(old_size))
			{	
				r = remove_block(oi);
	//			eprintk("Are you kidding me\n");
			} 
			
			oi->oi_size = old_size;
	//		eprintk("Just kidding, not using the remove_block right now\n");
			return -ENOSPC;
		}
		else if(r == -EIO)
			return r;
...
ospfs_notify_change(struct dentry *dentry, struct iattr *attr)
{
	...
	int retval = 0;
	//eprintk("Calling ospfs_notify_change\n");
...
ospfs_read(struct file *filp, char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	while (amount < count && retval >= 0) {
		//call this function to return currect block number
		...
		//this function returns to the pointer pointing to the block
		data = ospfs_block(blockno);
		//compute the offset in this data block. We need to get the exact address of a block
		...
		/*
		if((data+offset)!= NULL)
			eprintk("read char %s",*(data+offset));
		*/
		if(retval)
		{
	//		eprintk("Copy_to_user failed when reading\n");
			return -EFAULT;
		}
	...
	//eprintk("Successfully read %d amount of data\n");
...
ospfs_write(struct file *filp, const char __user *buffer, size_t count, loff_t *f_pos)
{
	...
	//eprintk("The amount of data to write is %d\n", count);	

	if(*f_pos + count >= oi->oi_size){
		retval = change_size(oi, *f_pos + count);//this already changes oi
		if(retval < 0)
		{
	//		eprintk("Failed to change file size\n");
			return retval;
		}
	}
	...
	while (amount < count && retval >= 0) {
		...
		uint32_t offset;
		char *data;
		//eprintk("The we are looking for %dth byte in a block\n", *f_pos);
		if (blockno == 0) {
			retval = -EIO;
	//		eprintk("Blockno error\n");
			goto done;
		}
		//eprintk("The amount is now %d\n", amount);
		//eprintk("You are at position %d\n", *f_pos);
		data = ospfs_block(blockno);
	//	eprintk("We are at block number %d\n", blockno);
		
		/*
		char *buffer_ptr = buffer;
		while (buffer_ptr!=NULL){
			eprintk("The data is %s\n", *buffer_ptr);
			buffer_ptr++;
		}
		*/
		offset = *f_pos % OSPFS_BLKSIZE;
		//eprintk("The offset is %d\n", offset);
		
		//how much byte left empty within the current block
		n = OSPFS_BLKSIZE - offset;
	//	eprintk("There are %d bytes left empty in this block\n", n);
		//eprintk("copy from  %s to %s\n", *(buffer), *(data+offset));
		...
		if(n > count - amount)
			n = count - amount; 
	//	eprintk("Need to copy %d bytes \n", n);

		if(copy_from_user(data + offset,buffer, n) != 0)
                {
//			eprintk("Failed to use copy_to_user in writing\n");
                        return -EFAULT;
                }
...
//	eprintk("Finish doing ospfs_write\n");
	return (retval >= 0 ? amount : retval);
}
...
{
//	eprintk("We gonna use function create_blank_direntry\n");
	// Outline:
...
{
//	eprintk("Gonna use the function ospfs_create\n");
	ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
	...
	if(dir_oi -> oi_ftype != OSPFS_FTYPE_DIR)
		return -EIO;
	if(dentry->d_name.len > OSPFS_MAXNAMELEN){
		return -ENAMETOOLONG; // Replace this line
	}
...
{
//	eprintk("Gonna use the function ospfs_symlink\n");
	ospfs_inode_t *dir_oi = ospfs_inode(dir->i_ino);
	uint32_t entry_ino = 0;
	ospfs_direntry_t *new_entry = NULL;	
	ospfs_symlink_inode_t *symblink;
	char *qmark;
	char *colon;
	...
	symblink = (ospfs_symlink_inode_t *)ospfs_inode(entry_ino);
	if(symblink == NULL)
		return -EIO;
	new_entry = create_blank_direntry(dir_oi);
	if(IS_ERR(new_entry))
	...
	{
		size_t root_path = colon - qmark + 1;
		size_t rest = strlen(colon);
		if(root_path + rest > OSPFS_MAXNAMELEN)
			return -ENAMETOOLONG;
		symblink->oi_size = strlen(qmark) + 1;
		strncpy(symblink->oi_symlink, qmark, root_path -1);
		symblink->oi_symlink[root_path - 1] = '\0';
		strncpy(symblink->oi_symlink + root_path, colon, rest);
		symblink->oi_symlink[symblink->oi_size] = '\0';
	}
	else
	{
		uint32_t length = strlen(symname);
		if(length > OSPFS_MAXSYMLINKLEN)
			return -ENAMETOOLONG;
		symblink->oi_size = length;
		strncpy(symblink->oi_symlink, symname, symblink->oi_size);
		symblink->oi_symlink[symblink->oi_size] = '\0';
	}
	symblink->oi_ftype = OSPFS_FTYPE_SYMLINK;
	symblink->oi_nlink = 1;
...
{
//	eprintk("Gonna use the function ospfs_follow_link\n");
